...
use qubit_redact::{
    http::{HttpFieldContext, HttpRedactionPolicy, HttpRedactor, UrlPathPolicy},
    Sensitivity,
};
use std::str::FromStr;
use url::Url;
...
impl HttpClientOptions {
        ...
        R: ConfigReader + ?Sized,
    {
        ...
        {
            let timeouts_config = config.section("timeouts").map_err(HttpConfigError::from)?;
        ...
        if config
            ...
            .map_err(HttpConfigError::from)?
        {
            let proxy_config = config.section("proxy").map_err(HttpConfigError::from)?;
        ...
        if config
            ...
            .map_err(HttpConfigError::from)?
        {
            let logging_config = config.section("logging").map_err(HttpConfigError::from)?;
        ...
        if config
            ...
            .map_err(HttpConfigError::from)?
        {
            let retry_config = config.section("retry").map_err(HttpConfigError::from)?;
        ...
        if config
            ...
            .map_err(HttpConfigError::from)?
        {
            let sse_config = config.section("sse").map_err(HttpConfigError::from)?;
            ...
            .map_err(HttpConfigError::from)?
        {
            ...
            let mut policy_builder = HttpRedactionPolicy::default().to_builder();
            ...
            if let Some(names) = log_redaction.sensitive_headers {
                for name in names {
                    policy_builder = policy_builder
                        .raise(HttpFieldContext::Header, &name, Sensitivity::High)
                        .map_err(|error| {
            ...
            if let Some(names) = log_redaction.sensitive_query_params {
                for name in names {
                    policy_builder = policy_builder
                        .raise(HttpFieldContext::Query, &name, Sensitivity::High)
                        .map_err(|error| {
                            Self::resolve_config_error(
                                &log_redaction_config,
                                HttpConfigError::invalid_value(
