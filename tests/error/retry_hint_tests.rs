...
fn test_retry_hint_for_status_code() {
    let too_many_requests = HttpError::from_status(StatusCode::TOO_MANY_REQUESTS, "429");
    let internal_error = HttpError::from_status(StatusCode::INTERNAL_SERVER_ERROR, "500");
    let bad_request = HttpError::from_status(StatusCode::BAD_REQUEST, "400");
...
fn test_retry_hint_non_retryable_for_protocol_and_config_errors() {
    ...
    let retry_attempt_timeout = HttpError::retry_budget_exceeded("attempt timeout");
    let retry_max_elapsed = HttpError::retry_budget_exceeded("max elapsed");
    let retry_aborted = HttpError::other("aborted");
