...
fn test_http_client_options_invalid_ipv4_only_type_is_prefixed() {
    ...
    let err = HttpClientOptions::from_config(&config.prefix_view("http"))
        .unwrap_err();
...
fn test_http_client_options_reqwest_extra_fields_from_config() {
    ...
    let opts =
        HttpClientOptions::from_config(&config.prefix_view("http")).unwrap();
...
fn test_http_retry_options_delay_strategies_from_config() {
    ...
    fixed_config
        ...
        .unwrap();
    let fixed =
        HttpRetryOptions::from_config(&fixed_config.prefix_view("retry"))
            .unwrap();
    assert_eq!(
    ...
    random_config
        ...
        .unwrap();
    let random =
        HttpRetryOptions::from_config(&random_config.prefix_view("retry"))
            .unwrap();
    assert_eq!(
...
fn test_http_retry_options_method_policy_aliases_from_config() {
    ...
    idempotent_config
        ...
        .unwrap();
    let idempotent =
        HttpRetryOptions::from_config(&idempotent_config.prefix_view("retry"))
...
fn test_http_retry_options_invalid_method_policy_from_config() {
    ...
    let err = HttpRetryOptions::from_config(&config.prefix_view("retry"))
        .unwrap_err();
...
fn test_http_client_options_sse_done_marker_default_alias_and_custom_value() {
    ...
    let default_opts =
        HttpClientOptions::from_config(&default_config.prefix_view("http"))
            .unwrap();
    assert_eq!(
    ...
    let custom_opts =
        HttpClientOptions::from_config(&custom_config.prefix_view("http"))
            .unwrap();
    assert_eq!(
