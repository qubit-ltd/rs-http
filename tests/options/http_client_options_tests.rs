...
fn test_http_client_options_invalid_ipv4_only_type_is_prefixed() {
    ...
    let err = HttpClientOptions::from_config(&config.section("http"))
        .unwrap_err();
...
fn test_http_client_options_logging_section() {
    ...
    config
        .set("http.logging.body_size_limit", 8192u64)
        .unwrap();
...
fn test_http_retry_options_delay_strategies_from_config() {
    ...
    fixed_config
        ...
        .unwrap();
    let fixed =
        HttpRetryOptions::from_config(&fixed_config.section("retry"))
            .unwrap();
    assert_eq!(
    ...
    random_config
        ...
        .unwrap();
    let random =
        HttpRetryOptions::from_config(&random_config.section("retry"))
            .unwrap();
    assert_eq!(
    ...
    none_config
        ...
        .unwrap();
    let none = HttpRetryOptions::from_config(&none_config.section("retry"))
        .unwrap();
...
fn test_http_retry_options_invalid_method_policy_from_config() {
    ...
    let err = HttpRetryOptions::from_config(&config.section("retry"))
        .unwrap_err();
...
fn test_http_client_options_sse_done_marker_default_alias_and_custom_value() {
    ...
    let custom_opts =
        HttpClientOptions::from_config(&custom_config.section("http"))
            .unwrap();
    assert_eq!(
...
fn test_http_client_options_from_root_config_all_sections() {
    ...
    config.set("ipv4_only", true).unwrap();
    config
        .set("error_response_preview_limit", 1024u64)
        .unwrap();
    config
