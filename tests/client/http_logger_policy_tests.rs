...
use qubit_http::{
    ...
    HttpRequest, HttpRequestBody, HttpRequestBodyByteStream, HttpResponse, HttpResponseMeta,
    LogRedactionPolicy,
};
...
fn test_log_stream_response_headers_respects_toggle() {
    ...
    assert!(logs.contains("<-- 200 https://example.com/%3Credacted%3E (stream)"));
...
fn test_log_request_includes_builder_query_params() {
    ...
    assert!(logs.contains("--> GET https://example.com/%3Credacted%3E?existing=1&added=two+words"));
}
